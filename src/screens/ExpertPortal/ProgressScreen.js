...
const ProgressScreen = () => {
  ...
  const downloadPDF = async () => {
    try {
      ...
      const { uri } = await Print.printToFileAsync({
        html: generateHTML(),
        base64: false, // Don't encode in base64
      });
  
      // Define a path to save the PDF
      const filePath = `${FileSystem.documentDirectory}progress_report.pdf`;
      ...
      if (await Sharing.isAvailableAsync()) {
        ...
        alert('File saved but sharing is not available on this device.');
      }
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      <TouchableOpacity style={styles.shareButton} onPress={downloadPDF}>
        <Text style={styles.shareButtonText}>Download Report</Text>
        <Icon name="download" size={20} color="#FFF" style={styles.shareIcon} />
      </TouchableOpacity>
